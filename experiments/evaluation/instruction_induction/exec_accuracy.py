...
def exec_accuracy_evaluator(prompts, eval_template, eval_data, demos_template, few_shot_data, config):
    ...
    for prompt in prompts:
        ...
        for d in zip(*subsampled_data):
            ...
            print("Query: ",query)
            # TODO: ensure that there are few shots of example
            answers.append(output_)
            print("Ground Truth answer:", output_)
            print("\n")

    # Instantiate the LLM
    model = llm.Llama2Model(config, disable_tqdm=True)
    model_outputs = model.make_response(queries,parse=True)

    
    for out in model_outputs:
        print("Model_output",out)
    task = config['task']
