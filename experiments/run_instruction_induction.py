...
sub_tasks = ['antonyms', 'cause_and_effect', 'common_concept', 'diff', 'first_word_letter',
             ...
             'translation_en-fr', 'word_in_context','gsm8k']
...
def run(task):
    ...
    prompt_gen_template = "I gave a friend a instruction. Based on the instruction they produced " \
                          "the following input-output pairs:\n\n[full_DEMO]\n\nObserve the input-output pair,think step by step and then output the instruction. The final answer should start with:\nThe instruction was to [APE]"
    ...
    conf = {
        ...
        'evaluation': {
            ...
            'task': task,
            'num_samples': min(10, len(eval_data[0])),
            # 'model': {
            #     'gpt_config': {
            #         # 'model': 'text-ada-001'
            #     }
