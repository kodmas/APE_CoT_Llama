...
def demo_function(eval_template, config):
    ...
    def fn(prompt, inputs):
        ...
        outputs = model.make_response(queries)
        reply_strings = model.tokenizer.batch_decode(outputs.sequences, skip_special_tokens=True)
        return [out.strip().split('\n')[0] for out in reply_strings]
