...
def generate_prompts(prompt_gen_template, demos_template, prompt_gen_data, config):
    ...
    # Instantiate the LLM
    model = llm.Llama2Model(config, disable_tqdm=False)
    #prompts = model.make_response(queries)
    prompts = model.generate_text(
         queries, n=config['num_prompts_per_subsample'])
    return prompts

def generate_llama2_prompts(prompt_gen_template, demos_template, prompt_gen_data, config):
